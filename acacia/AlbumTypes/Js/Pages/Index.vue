<template>
    <Head>
        <title>Album Types</title>
    </Head>
    <Backend>
        <template #header>
            <h4 class="font-black text-2xl px-1 md:px-4">Album Types</h4>
        </template>
        ...
        >
            ...
            <template #footer>
                <Button
                    ...
                    @click="
                        $inertia.visit(
                            route('acacia.backend.album-types.create')
                        )
                    "
                    class="p-button-text"
                />
            ...
            <template #footer>
                <Button
                    ...
                    @click="
                        $inertia.visit(
                            route(
                                'acacia.backend.album-types.show',
                                currentModel
                            )
                        )
                    "
            ...
            <template #footer>
                <Button
                    ...
                    @click="
                        $inertia.visit(
                            route(
                                'acacia.backend.album-types.edit',
                                currentModel
                            )
                        )
                    "
...
const fetchModel = async (row) => {
    axios
        .get(route("api.v1.album-types.show", row))
        .then((res) => {
...
const deleteModel = async function (row) {
    try {
        const res = await axios.delete(
            route("api.v1.album-types.destroy", row as any)
        );
        toast.add({
            severity: "success",
